...
- When a migration file has never been committed, it means it has not moved to production. That means you can actually edit that file to add more things to it instead of creating a new migration file just to make adjustments to the table. So always feel free to edit the contents of a migration file if it has never been committed. Also put related migration actions into the same migration file. eg, creation of similar tables like posts and comments should be in the same migration file.
