...
it('creates an activity log with the service', function () {
    ...
    expect($log)->toBeInstanceOf(ActivityLog::class)
        ...
        ->and($log->actor_name)->toBe($user->full_name)
        ->and($log->subject_id)->toBe($institution->id)
        ->and($log->subject_name)->toBe($institution->name)
        ->and($log->properties->toArray())->toBe(['key' => 'value']);
});
...
it('logs guest and system activity without an actor', function () {
    ...
    expect($log->actor_type)->toBeNull()
        ...
        ->and($log->actor_name)->toBeNull()
        ->and($log->event)->toBe('audit.system_event');
});
...
it('captures request context during an http request', function () {
    ...
    expect($log->actor_id)->toBe($user->id)
        ->and($log->route_name)->toBe('audit.context-test')
        ->and($log->method)->toBe('GET')
        ->and($log->request_id)->toBe('request-123')
        ->and($log->url)->toContain('/audit-context-test');
});
...
it('redacts sensitive keys in payloads and diffs', function () {
    ...
    expect($log->properties->toArray())->toBe([
        ...
        ->and($log->old_values->toArray())->toBe([
            'token' => ActivityLogSanitizer::REDACTED,
        ])
        ->and($log->new_values->toArray())->toBe([
...
it('blocks cross-institution audit log access', function () {
    ...
    actingAs($institution->createdBy)
        ->getJson(route('institutions.activity-logs.index', $otherInstitution->uuid))
        ->assertForbidden();
});
