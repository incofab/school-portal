...
it(
    ...
    function () {
        ...
        // Assert Theory Questions
        expect($formattedCourseSession)->toHaveKey('theory_questions');
        expect($formattedCourseSession['theory_questions'])
            ->toBeArray()
            ->toHaveCount(2);
        $sampleTheoryQuestion = $courseSession->theoryQuestions->first();
        $formattedTheoryQuestion = collect(
            $formattedCourseSession['theory_questions']
        )->firstWhere('id', $sampleTheoryQuestion->id);
        expect($formattedTheoryQuestion)
            ->toHaveKey('id', $sampleTheoryQuestion->id)
            ->toHaveKey('question_no', $sampleTheoryQuestion->question_no)
            ->toHaveKey(
                'question_sub_number',
                $sampleTheoryQuestion->question_sub_number
            )
            ->toHaveKey('question', $sampleTheoryQuestion->question)
            ->toHaveKey('marks', $sampleTheoryQuestion->marks)
            ->toHaveKey('answer', $sampleTheoryQuestion->answer)
            ->toHaveKey('marking_scheme', $sampleTheoryQuestion->marking_scheme)
            ->toHaveKey('course_session_id', $courseSession->id);
