...
it('allows an admin to create a class-targeted uploaded library material', function () {
    $file = UploadedFile::fake()->create('lesson-note.pdf', 256, 'application/pdf');
...
it('only shows students public and matching class library materials', function () {
...
it('allows the owner teacher to update and delete a library material', function () {
    ...
    actingAs($this->teacher)
        ->putJson(
            route('institutions.libraries.update', [
                $this->institution,
                $library,
            ]),
            [
    ...
    actingAs($this->teacher)
        ->deleteJson(route('institutions.libraries.destroy', [
            $this->institution,
            $library,
        ]))
        ->assertOk();
