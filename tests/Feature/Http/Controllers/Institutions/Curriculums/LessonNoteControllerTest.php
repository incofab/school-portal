...
beforeEach(function () {
    ...
    $this->courseTeacher = CourseTeacher::factory()
        ->withInstitution($this->institution)
        ->create(['course_id' => $this->course->id, 'classification_id' => $this->classification->id]);
    ...
    $this->lessonNote = LessonNote::factory()
        ...
        ->create(['course_id' => $this->course->id, 'course_teacher_id' => $this->courseTeacher->id]);
...
it('stores lesson note data', function () {
    ...
    $dLessonNote = LessonNote::factory()
        ->lessonPlan($this->lessonPlan, $this->classification)->make([
...
it('restricts students access to some lesson note routes', function () {
    ...
    $routes = [
        ...
        'create' => route('institutions.lesson-notes.create', ['institution' => $this->institution->uuid, 'lessonPlan' => $this->lessonPlan->id]),
        'edit' => route('institutions.lesson-notes.edit', ['institution' => $this->institution->uuid, 'lessonNote' => $lessonNote->id]),
        'store' => route('institutions.lesson-notes.store-or-update', ['institution' => $this->institution->uuid]),
        'update' => route('institutions.lesson-notes.store-or-update', ['institution' => $this->institution->uuid, 'lessonNote' => $lessonNote->id]),
        'destroy' => route('institutions.lesson-notes.destroy', ['institution' => $this->institution->uuid, 'lessonNote' => $lessonNote->id]),
    ];
    ...
    foreach ($routes as $name => $route) {
        ...
        actingAs($this->student->user)
            ->$method($route, $name === 'store' || $name === 'update' ? ['content' => 'Test'] : [])
            ->assertForbidden();
    }
});
