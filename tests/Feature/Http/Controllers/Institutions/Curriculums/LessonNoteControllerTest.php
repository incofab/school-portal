...
it('uploads lesson note media one file at a time', function () {
  ...
  $route = route('institutions.lesson-notes.media.store', [
    'institution' => $this->institution->uuid,
  ...
  assertDatabaseHas('media', [
    'mediable_type' => $this->lessonNote->getMorphClass(),
    'mediable_id' => $this->lessonNote->id,
    'collection_name' => 'attachments'
  ]);
});

it('deletes lesson note media', function () {
  Storage::fake('s3_public');
  ...
  $res = app(\App\Support\Media\MediaManager::class)->storeUploadedFile(
    ...
    $this->institution->folder(
      \App\Enums\S3Folder::LessonNotes,
      (string) $this->lessonNote->id
    ),
    $this->institution,
    $this->admin
  );
  ...
  $route = route('institutions.lesson-notes.media.destroy', [
    'institution' => $this->institution->uuid,
    'lessonNote' => $this->lessonNote->id,
    'media' => $media->id
  ]);
