...
it('restricts non-admin access to lesson plan routes', function () {
    ...
    $routes = [
        'create' => route('institutions.lesson-plans.create', ['institution' => $this->institution->uuid, 'schemeOfWork' => $this->schemeOfWork->id]),
        'edit' => route('institutions.lesson-plans.edit', ['institution' => $this->institution->uuid, 'lessonPlan' => $lessonPlan->id]),
        'store' => route('institutions.lesson-plans.store-or-update', ['institution' => $this->institution->uuid]),
        'update' => route('institutions.lesson-plans.store-or-update', ['institution' => $this->institution->uuid, 'lessonPlan' => $lessonPlan->id]),
        'destroy' => route('institutions.lesson-plans.destroy', ['institution' => $this->institution->uuid, 'lessonPlan' => $lessonPlan->id]),
    ];
    ...
    foreach ($routes as $name => $route) {
        ...
        actingAs($this->user)
            ->$method($route, $name === 'store' || $name === 'update' ? ['title' => 'Test'] : [])
            ->assertForbidden();
    }
});
