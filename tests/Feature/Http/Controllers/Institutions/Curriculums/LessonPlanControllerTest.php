...
it('lists and filters lesson plans', function () {
  $this->topic->update([
    'course_id' => $this->course->id,
    'classification_group_id' => $this->classificationGroup->id
  ]);
  $this->courseTeacher->update([
  ...
  $lessonPlan = LessonPlan::factory()
    ...
    ->create([
      ...
      'objective' => 'Teach number bonds'
    ]);

  LessonPlan::factory()->create(['objective' => 'Unrelated objective']);

  $route = route('institutions.lesson-plans.index', [
    'institution' => $this->institution->uuid,
    'classificationGroup' => $this->classificationGroup->id,
    'classification' => $this->classification->id,
    'course' => $this->course->id,
    'courseTeacher' => $this->courseTeacher->id,
    'search' => 'number bonds'
  ]);
  ...
  actingAs($this->admin)
    ...
    ->assertInertia(
      fn(AssertableInertia $assert) => $assert
        ->component('institutions/lesson-plans/list-lesson-plans')
        ->has('lessonPlans.data', 1)
        ->where('lessonPlans.data.0.id', $lessonPlan->id)
    );
});
...
it('deletes lesson plan media', function () {
  ...
  $res = app(\App\Support\Media\MediaManager::class)->storeUploadedFile(
    UploadedFile::fake()->image('plan.jpg'),
    $lessonPlan,
    'attachments',
    $this->institution->folder(
      \App\Enums\S3Folder::LessonPlans,
      (string) $lessonPlan->id
    ),
    $this->institution,
    $this->admin
  );
