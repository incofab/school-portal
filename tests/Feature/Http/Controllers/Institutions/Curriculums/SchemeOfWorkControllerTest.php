...
it('lists and filters scheme of works', function () {
    ...
    $schemeOfWork = SchemeOfWork::factory()
        ->topic($topic)
        ->create(['term' => TermType::First->value, 'week_number' => 4]);

    SchemeOfWork::factory()
        ->topic($this->topic)
        ->create(['term' => TermType::Second->value]);

    $route = route('institutions.scheme-of-works.index', [
        'institution' => $this->institution->uuid,
        'classificationGroup' => $classificationGroup->id,
        'course' => $course->id,
    ...
    actingAs($this->admin)
        ...
        ->assertInertia(
            fn (AssertableInertia $assert) => $assert
                ->component('institutions/scheme-of-works/list-scheme-of-works')
                ->has('schemeOfWorks.data', 1)
                ->where('schemeOfWorks.data.0.id', $schemeOfWork->id)
        );
});
