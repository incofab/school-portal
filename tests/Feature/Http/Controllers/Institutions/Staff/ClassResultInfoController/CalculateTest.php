...
it('allows an admin to generate AI comments for a class result info record', function () {
    $classResultInfo = ClassResultInfo::factory()
        ->classification($this->classA)
        ->create([
            'academic_session_id' => $this->academicSession->id,
    ...
    app()->instance(
        GenerateAiTermResultComments::class,
        new class extends GenerateAiTermResultComments
        {
    ...
    actingAs($this->instAdmin)
        ->postJson(
            route('institutions.class-result-info.generate-ai-comments', [
                $this->institution->uuid,
                $classResultInfo,
            ])
        )
...
it('prevents a teacher who is not the form teacher from generating AI comments', function () {
