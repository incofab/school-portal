...
it('renders the create attendance view for authorized users', function () {
    ...
    actingAs($this->admin)->get($route)->assertOk();
});


// Test attendance store with 'sign-in' type
it('allows authorized user to store a sign-in attendance record', function () {
    $institutionStaffUser = $this->admin->institutionUsers()->where('institution_id', $this->institution->id)->first();
    ...
    actingAs($this->admin)->postJson($route, $payload)
        ->assertOk();
...
it('allows authorized user to store a sign-out attendance record if signed in', function () {
    $remark = 'Existing remark';
    $signInAttendance = Attendance::factory()->signedInOnly()->institutionUser($this->institutionUser)->create([
        'remark' => $remark
    ]);
...
it('returns an error when signing out without a previous signed-in record', function () {
    $payload = [
        'institution_id' => $this->institution->id,
        'institution_user_id' => $this->institutionUser->id,
        'type' => AttendanceType::Out->value,
...
it('prevents a non-admin user from deleting an attendance record', function () {
    ...
    actingAs($this->institutionUser->user)->deleteJson($route)->assertStatus(403);
});