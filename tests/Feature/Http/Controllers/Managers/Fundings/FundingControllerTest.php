...
beforeEach(function () {
    $this->user = User::factory()->adminManager()->create();
    $this->institutionGroup = InstitutionGroup::factory()->create(['credit_wallet' => 1000]);
});
...
it('creates funding and updates wallet balance', function () {
    ...
    actingAs($this->user)->postJson(route('managers.funding.store'), $payload)
        ->assertOk();
