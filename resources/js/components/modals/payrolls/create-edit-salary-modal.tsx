...
export default function CreateEditSalaryModal({
  isOpen,
  onSuccess,
  onClose,
  salary,
  ...
  const webForm = useWebForm({
    ...
    institution_user_id: salary?.institution_user
      ? {
          label: salary.institution_user.user?.full_name,
          value: salary.institution_user_id,
        }
      : null,
  });

  const isUpdate = salary?.id;
  const parentSalaryType = getParentSalaryType(webForm.data.salary_type_id);
  function getParentSalaryType(salaryTypeId?: number | string) {
    if (!salaryTypeId) return null;
    return salaryTypes.find((item) => item.id === salaryTypeId)?.parent;
  }

  const onSubmit = async () => {
    const res = await webForm.submit(async (data, web) => {
      return isUpdate
        ? web.put(instRoute('salaries.update', [salary]), data)
        : web.post(instRoute('salaries.store'), {
            ...data,
