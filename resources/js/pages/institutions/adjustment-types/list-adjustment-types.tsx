...
interface Props {
    adjustmentTypes: PaginationResponse<AdjustmentType>;
    parentAdjustmentTypes: AdjustmentType[];
}

export default function ListStaffTypes({ adjustmentTypes, parentAdjustmentTypes }: Props) {

    const { instRoute } = useInstitutionRoute();
    const userFilterToggle = useModalToggle();
    const editAdjustmentTypeModal = useModalValueToggle<AdjustmentType | undefined>();

    const deleteForm = useWebForm({});
    const { handleResponseToast } = useMyToast();

    async function deleteItem(obj: AdjustmentType) {
        const res = await deleteForm.submit((data, web) =>
            web.delete(instRoute('adjustment-types.destroy', [obj.id]))
        );
        handleResponseToast(res);
        Inertia.reload();
    }

    const headers: ServerPaginatedTableHeader<AdjustmentType>[] = [
        {
            label: 'Title',
        ...
        {
            label: 'Percentage',
            render: (row) => <Text whiteSpace={'nowrap'}>{row.percentage ? `${row.percentage}% of ${row.parent?.title}` : ''}</Text>,
        },
        {
            label: 'Description',
    ...
    function openModal(adjustmentType?: AdjustmentType) {
        editAdjustmentTypeModal.open(adjustmentType ?? {} as AdjustmentType);
    }
        ...
        <DashboardLayout>
            <Slab>
                ...
                <SlabBody>
                    <ServerPaginatedTable
                        scroll={true}
                        headers={headers}
                        data={adjustmentTypes.data}
                        keyExtractor={(row) => row.id}
                        validFilters={['role']}
                        paginator={adjustmentTypes}
            ...
            {editAdjustmentTypeModal.state != undefined &&
                <CreateEditAdjustmentTypeModal
                    adjustmentType={editAdjustmentTypeModal.state ?? null}
