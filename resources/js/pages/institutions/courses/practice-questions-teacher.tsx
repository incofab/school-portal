...
import { HStack, Text, VStack, Box, Checkbox, Button } from '@chakra-ui/react';
...
export default function PracticeQuestionsTeacher({
  practiceQuestions,
  course,
}: Props) {
  const [selectedQuestions, setSelectedQuestions] = useState<
    PracticeQuestion[]
  >([]);
  const insertQuestionsIntoQuestionbankModalToggle =
    useModalValueToggle<Course>();
      ...
      <Slab>
        ...
        <SlabBody>
              ...
              <VStack align="start" spacing={1}>
                <VStack align="stretch" ml={10}>
                  {(
                    ...
                    return <Text key={optionKey}>{q[optionKey]}</Text>;
                  })}
                </VStack>
              </VStack>
            </Box>
          ...
          <Button
            colorScheme={'brand'}
            onClick={() =>
              insertQuestionsIntoQuestionbankModalToggle.open(courseData)
            }
            mt={4}
      ...
      {insertQuestionsIntoQuestionbankModalToggle.state && (
        <InsertQuestionsIntoQuestionbankModal
          ...
          onSuccess={(newCourseData) => {
            newCourseData && setCourseData(newCourseData);
          }}
        />
      )}
    </DashboardLayout>
