...
export default function RecruitmentExamLogin() {
  ...
  async function handleSubmit() {
    const res = await webForm.submit((data, web) =>
      web.post(route('recruitment.exam.login.store'), data)
    );
  ...
  return (
    <CenteredLayout title="Recruitment Exam Login">
      <VStack
        spacing={4}
        align={'stretch'}
        as={'form'}
      ...
      >
        ...
        <FormControl isInvalid={!!webForm.errors.application_no}>
          <FormLabel>Recruitment no</FormLabel>
          <Input
            type="text"
