...
class HandlePaymentCallback
{
  ...
  function handlePaystackCallback($post)
  {
    ...
    if (!$paymentRef) {
      return ret(false, 'Paystack record not found');
    }
    ...
    if ($paymentRef['payment_type'] == SubscriptionPlan::class) {
      $ret = $this->subscriptionHelper->recordCardPayment(
        $paymentRef,
        MERCHANT_PAYSTACK
      );
      ...
      $ret = $this->depositHelper->recordCardPayment(
        $paymentRef,
        $amount,
        MERCHANT_PAYSTACK
      );
  ...
  function handleRaveCallback($post)
  {
    ...
    $paymentRef = PaymentReference::where('reference', '=', $ref)
      ->with(['user'])
      ->first();

    if (!$paymentRef) {
      return ret(false, 'Invalid reference');
    }
  ...
  function handleMonnifyCallback($post)
  {
    ...
    if (!$paymentRef) {
      return ret(false, 'Monnify payment record not found');
    }
