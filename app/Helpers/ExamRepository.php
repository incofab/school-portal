...
class ExamRepository
{
  ...
  public function __construct(FormatExam $formatExam, ExamHandler $examHandler)
  {
  ...
  {
    $courses = Course::where('exam_content_id', '=', $examBodyId)
      ->with(['sessions', 'topics'])
      ->get();
  ...
  function registerExam(
    ...
    $student = Student::whereInstitution_id($institution->id)
      ->whereStudent_id($studentId)
      ->first();

    if (!$student) {
      return retF('Student record not found in this institution');
    }
  ...
  function pauseExam($examNo)
  {
    ...
    if (!$exam['end_time']) {
      ...
      Carbon::parse($exam['end_time'])->getTimestamp() <
  ...
  function endExam($examNo)
  {
    /** @var Exam $exam */
    $exam = Exam::whereExam_no($examNo)
      ->with(['examSubjects', 'event', 'student'])
      ->first();
    ...
    if ($exam['status'] == 'ended') {
      return retF('Exam already submitted');
    }
  ...
  function startExam($examNo)
  {
    ...
    $examSubjects = ExamSubject::whereExam_no($examNo)
      ->with(['course', 'session'])
      ->get();

    $arr = [];

    $contentRet = $this->examHandler->getContent(
      $exam['event_id'],
      $exam['exam_no'],
      false
    );

    $allQuestionAttempts = $contentRet[SUCCESSFUL]
      ? $contentRet['content']['attempts']
      : null;
    ...
    if ($exam['status'] == 'paused') {
      $timeElapsed = Carbon::parse($exam['start_time'])->diffInSeconds(
        Carbon::parse($exam['pause_time']),
        false
      );
    ...
    $arr['meta']['time_remaining'] = Carbon::now()->diffInSeconds(
  ...
  {
    if ($mins < 1) {
      return retF('Set a valid extension time');
    }
    ...
    if (
      $exam['status'] === 'ended' ||
      \Carbon\Carbon::now()->diffInSeconds($currentEndTime, false) < 10
    ) {
    ...
    if ($exam['status'] === 'paused' && !empty($exam['pause_time'])) {
    ...
    return retS(
      "Exam time has been extende by {$mins}mins. Student should refresh his/her page."
    );
  }
  ...
  function list(
    ...
    $allRecords = $allRecords
      ->with(['examSubjects', 'examSubjects.course', 'user'])
      ->orderBy('id', 'DESC')
      ->skip($num * ($page - 1))
      ->take($num)
      ->get();
