...
class ExamHandler
{
  ...
  function attemptQuestion(array $studentAttempts, $eventId, $examNo)
  {
    ...
    if ($ret === false) {
      return [
        'success' => false,
        'message' => 'Exam file failed to recorded attempt'
      ];
    }

    return [
      'success' => true,
      'message' => 'Exam file, question attempt recorded'
    ];
  }

  function calculateScoreFromFile($exam, $examSubjectId, $questions)
  {
    ...
    foreach ($questions as $question) {
      if (!empty($subjectAttempts[$question['id']]['attempt'])) {
        if (
          $question['answer'] === $subjectAttempts[$question['id']]['attempt']
        ) {
          $score++;
        }
      }
  ...
  private function getFullFilepath(
    ...
    $baseFolder = "../public/exams/$examFolderName";
