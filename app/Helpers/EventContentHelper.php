...
{
  private $baseDir = '../public/events-export/';
  ...
  function downloadEventContent($post)
  {
    ...
    if (!$institution) {
      return retF('Institution not found');
    }
    ...
    if (!$eventSubjects->first()) {
      return retF('Event subject empty');
    }

    $exams = Exam::where('event_id', $event->id)
      ->with(['student', 'examSubjects'])
      ->get();

    if (!$exams->first()) {
      return retF('Exams empty');
    }
    ...
    foreach ($eventSubjects as $eventSubject) {
      $session = CourseSession::where('id', $eventSubject->course_session_id)
        ->with(['course', 'questions', 'instructions', 'passages'])
        ->first();
    ...
    foreach ($exams as $exam) {
      file_put_contents(
        "$baseFolder/exam-{$exam->exam_no}.json",
        json_encode($exam->toArray(), JSON_PRETTY_PRINT)
      );
    }

    file_put_contents(
      "$baseFolder/event.json",
  ...
  private function copySessionImgs(
    $courseId,
    $course_session_id,
    $destinationBaseFolder
  ) {
    $containingfolder = "../public/img/content/$courseId/$course_session_id";
    $destinationBaseFolder = "$destinationBaseFolder/$courseId/$course_session_id";

    if (!file_exists($containingfolder)) {
      return;
    } //Nothing to copy
