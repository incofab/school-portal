...
class MediaManager
{
    ...
    public function registerExistingFile(
        ...
        $media = Media::query()->create([
            ...
            'collection_name' => $collectionName,
            'disk' => $disk,
            'directory' => Str::of($path)->beforeLast('/')->value() ?: null,
            'path' => $path,
    ...
    public function extractPathFromUrl(string $url, string $disk = 's3_public'): string
    {
        $baseUrl = rtrim(Storage::disk($disk)->url('/'), '/');

        $relativePath = ltrim((string) Str::of($url)->after($baseUrl)->value(), '/');
    ...
    private function makeFilename(string $originalName, ?string $extension): string
    {
