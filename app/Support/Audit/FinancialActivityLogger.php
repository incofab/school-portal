...
{
    public function feeRecorded(Fee $fee, string $action, array $oldValues = []): void
    {
        $fee->loadMissing('institution.institutionGroup', 'academicSession', 'feeCategories.feeable');

        $logger = $this->base("finance.fee.$action", ActivityLogCategory::Fee, $action)
            ->on($fee)
            ...
            ->severity($action === 'deleted' ? ActivityLogSeverity::Critical : ActivityLogSeverity::Notice);
    ...
    public function manualPaymentReviewed(ManualPayment $manualPayment, User $actor, bool $approved): void
    {
        $manualPayment->loadMissing('institution.institutionGroup', 'user', 'bankAccount', 'payable', 'paymentable');

        $action = $approved ? 'approved' : 'rejected';

        $this->base("finance.manual_payment.$action", ActivityLogCategory::Payment, $action)
            ->by($actor)
            ->on($manualPayment)
            ->inInstitution($manualPayment->institution)
            ->severity($approved ? ActivityLogSeverity::Critical : ActivityLogSeverity::Security)
    ...
    public function paymentNotificationSent(
        ...
        $this->base('finance.payment_notification.sent', ActivityLogCategory::Notification, 'sent')
            ->on($notification)
            ->inInstitution($fee->institution)
    ...
    public function providerWebhookReceived(
        string $provider,
        array $metadata,
        ?Institution $institution = null,
        ?Model $subject = null
    ): void {
        $this->base('finance.payment_webhook.received', ActivityLogCategory::Integration, 'webhook_received')
            ->on($subject)
            ->inInstitution($institution)
            ->description('External payment webhook received.')
            ->properties([
                'provider' => $provider,
                'metadata' => $metadata,
            ])
    ...
    public function paymentCredentialsChanged(
        ...
        $this->base('finance.payment_credentials.updated', ActivityLogCategory::Integration, 'updated')
            ->inInstitution($institution)
    ...
    public function withdrawalProcessed(Withdrawal $withdrawal, User $actor, string $status): void
    {
        $withdrawal->loadMissing('bankAccount', 'withdrawable');
        [$institution, $institutionGroup] = $this->contextFromModel($withdrawal->withdrawable);
        $action = $status === 'paid' ? 'approved' : 'rejected';

        $this->base("finance.withdrawal.$action", ActivityLogCategory::Wallet, $action)
            ->by($actor)
            ->on($withdrawal)
            ->inInstitution($institution)
    ...
    public function bankAccountChanged(
        ...
        $logger = $this->base("finance.bank_account.$action", ActivityLogCategory::Wallet, $action)
            ->on($bankAccount)
            ->inInstitution($institution)
            ->inInstitutionGroup($institutionGroup)
    ...
    {
        $commission->loadMissing('institutionGroup', 'partner.user', 'commissionable');

        $this->base('finance.commission.updated', ActivityLogCategory::Wallet, 'updated')
            ->on($commission)
            ...
            ->properties([
                ...
                'institution_group' => $this->modelSnapshot($commission->institutionGroup),
    ...
    public function payrollItemChanged(Salary $salary, string $action, array $oldValues = []): void
    {
        $salary->loadMissing('institution.institutionGroup', 'salaryType', 'institutionUser.user');

        $logger = $this->base("finance.payroll_item.$action", ActivityLogCategory::Payroll, $action)
            ->on($salary)
    ...
    public function expenseDecision(Expense $expense, string $action, array $oldValues = []): void
    {
        $expense->loadMissing('institution.institutionGroup', 'expenseCategory', 'institutionUser.user');

        $logger = $this->base("finance.expense.$action", ActivityLogCategory::Expense, $action)
            ->on($expense)
    ...
    private function base(string $event, ActivityLogCategory $category, string $action): ActivityLogger
    {
    ...
    private function credentialSummary(array $credentials): array
    {
        return collect($credentials)
            ->map(fn ($providerCredentials) => [
                'public_key_present' => filled($providerCredentials['public_key'] ?? null),
                'private_key_present' => filled($providerCredentials['private_key'] ?? null),
                'secret_key_present' => filled($providerCredentials['secret_key'] ?? null),
            ])
            ->all();
    }
