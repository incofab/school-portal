...
class ModelAuditRegistry
{
    ...
    public const MODELS = [
        ...
        InstitutionGroup::class => ['category' => ActivityLogCategory::InstitutionGroup],
        ...
        Classification::class => ['category' => ActivityLogCategory::Classification],
        ClassificationGroup::class => ['category' => ActivityLogCategory::Classification],
        ...
        AssignmentSubmission::class => ['category' => ActivityLogCategory::Assignment],
        ...
        AdmissionApplication::class => ['category' => ActivityLogCategory::Admission],
        AdmissionFormPurchase::class => ['category' => ActivityLogCategory::Admission],
        ApplicationGuardian::class => ['category' => ActivityLogCategory::Admission],
    ...
    {
        $category = self::MODELS[$model::class]['category'] ?? ActivityLogCategory::System;

        return $category instanceof ActivityLogCategory ? $category->value : $category;
    }
    ...
    public static function filterValues(array $values): array
    {
        return collect($values)
            ->reject(fn ($value, string $key) => in_array($key, self::IGNORED_ATTRIBUTES, true))
    ...
    private static function normalizeValue(string $key, mixed $value): mixed
    {
        ...
        if (is_array($value)) {
            return collect($value)
                ->map(fn ($item, $itemKey) => self::normalizeValue((string) $itemKey, $item))
                ->all();
        }
