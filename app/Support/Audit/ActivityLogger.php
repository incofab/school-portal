...
class ActivityLogger
{
  ...
  public function category(ActivityLogCategory|string $category): static
  {
    $this->payload['category'] =
      $category instanceof ActivityLogCategory ? $category->value : $category;
  ...
  public function inInstitutionGroup(
  ...
  {
    $this->payload['properties'] = ActivityLogSanitizer::sanitizeJsonArray(
      $properties
    );
  ...
  {
    $this->payload['old_values'] = ActivityLogSanitizer::sanitizeJsonArray(
      $oldValues
    );
  ...
  {
    $this->payload['new_values'] = ActivityLogSanitizer::sanitizeJsonArray(
      $newValues
    );
  ...
  public function severity(ActivityLogSeverity|string $severity): static
  {
    $this->payload['severity'] =
      $severity instanceof ActivityLogSeverity ? $severity->value : $severity;
  ...
  public function log(): ActivityLog
  {
    ...
    $payload = [
      ...
      'category' =>
        $this->payload['category'] ?? ActivityLogCategory::System->value,
      'event' => $this->payload['event'] ?? 'activity.logged',
      'severity' =>
        $this->payload['severity'] ?? ActivityLogSeverity::Info->value
    ];
  ...
  private function actorRole(Model $actor): ?string
  {
    if ($actor instanceof User) {
      ...
      $role = $actor->institutionUser()?->first()?->type;
