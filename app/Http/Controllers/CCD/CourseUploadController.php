...
{    
    ...
    function unInstallCourse($institutionId, $courseId)
    {
        ...
        return redirect(route('ccd.course.index', [$institutionId, $courseId]))
        ->with('message', $ret[MESSAGE]);
    }
	...
	private function uploadFile($files, $destinationPath = null) 
	{
	    ...
	    if($files['content']["error"] > 0) return [SUCCESSFUL => false, MESSAGE => "File upload error. Return Code: " . $files['content']["error"]];
