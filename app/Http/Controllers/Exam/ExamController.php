...
// 		    'student_data' => $studentDataFormated,
    ...
    {
        $examNo = $request->input('exam_no');
        if(!$examNo) return redirect(route('home.exam.view-result-form'))->with('message', 'Exam No not found');
        
        $exam = \App\Models\Exam::where('exam_no', '=', $examNo)
        ->with(['examSubjects', 'examSubjects.course', 'student', 'event', 'event.institution'])->first();
        
        if(!$exam) return redirect(route('home.exam.start'))->with('message', 'Exam not found');
        
        if($exam->status !== STATUS_ENDED && $exam->status !== STATUS_EXPIRED){
            return redirect(route('home.exam.start'))->with('message', 'Exam has not been concluded');
        }
        ...
        foreach ($examSubjects as $examSubject)
        {
            ...
            $scorePercent = \App\Core\Settings::getPercentage($score, $numOfQuestions, 0);
