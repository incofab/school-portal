...
class ClassResultInfoController extends Controller
{
    ...
    public function generateAiComments(
        Institution $institution,
        ClassResultInfo $classResultInfo,
        GenerateAiTermResultComments $generateAiTermResultComments
    ) {
        $classResultInfo->loadMissing('classification');
        $user = currentUser();
        abort_unless(
            currentInstitutionUser()->isAdmin() ||
              $classResultInfo->classification->form_teacher_id === $user->id,
            403,
            'You are not allowed to generate comments for this result'
        );

        $updatedCount = $generateAiTermResultComments->run($classResultInfo);

        return $this->ok(['updated_count' => $updatedCount]);
    }

    public function setNextTermResumptionDate(
        Institution $institution,
        Request $request,
    ...
    public function downloadClassResult(
        Institution $institution,
        ClassResultInfo $classResultInfo
    ) {
    ...
    public function fetchClassResultSheets(Request $request, Institution $institution)
    {
        $data = $request->validate([
