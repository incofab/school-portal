...
class AttendanceController extends Controller
{
    ...
    {
        $query = AttendanceUITableFilters::make($request->all(), Attendance::query())->filterQuery()->getQuery()
            ->with('institutionUser.user', 'institutionUser.student.classification')
            ->latest('attendances.id');
        return Inertia::render('institutions/attendances/list-institution-attendances', [
    ...
    public function store(Request $request, Institution $institution)
    {
        $validatedData = $request->validate([
            ...
            'reference' => [Rule::requiredIf($request->type === AttendanceType::In->value), function ($attr, $value, $fail) use ($request) {
        ...
        // if (request()->is('api/*')) {
        //     //== API request
        //     $staffUser = InstitutionUser::where('institution_id', $request->institution_id)
        ...
        if ($request->type === AttendanceType::In->value) {
            ...
            $lastSignIn = Attendance::where('institution_user_id', $request->institution_user_id)
