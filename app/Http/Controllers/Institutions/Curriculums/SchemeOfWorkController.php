...
use App\Http\Requests\Curriculums\SchemeOfWorkRequest;
...
class SchemeOfWorkController extends Controller
{
  ...
  public function index(Institution $institution, Request $request)
  {
    ...
    return Inertia::render(
      'institutions/scheme-of-works/list-scheme-of-works',
      [
  ...
  public function store(Institution $institution, SchemeOfWorkRequest $request)
  {
    $data = $request->validated();

    $params = [
      ...collect($data)
        ->except('is_used_by_institution_group', 'institution_id')
        ->toArray(),
  ...
  {
    if ($schemeOfWork->lessonPlans()->exists()) {
      return $this->message(
