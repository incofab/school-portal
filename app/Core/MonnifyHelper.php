...
class MonnifyHelper
{
  ...
  function auth()
  {
    ...
    if (!Arr::get($ret, SUCCESSFUL)) {
      return [
        ...
        MESSAGE =>
          'Error: ' .
          Arr::get($ret, MESSAGE, 'Base Server authentication failed')
      ];
    }
  ...
  {
    $create = \App\Models\ReservedAccount::insert(
      $userData['id'],
      MERCHANT_MONNIFY
    );
  ...
  function verifyReference($reference)
  {
    ...
    if (!$ret[SUCCESSFUL]) {
      return [SUCCESSFUL => false, MESSAGE => $ret[MESSAGE]];
    }
  ...
  function monnifyCallback(
    ...
    if (!$productType !== 'RESERVED_ACCOUNT') {
      return $handlePaymentCallback->handleMonnifyCallback([
        'reference' => $reference
      ]);
    }

    $reservedAcct = \App\Models\ReservedAccount::where(
      'reference',
      '=',
      $accountReference
    )
    ...
    if (!$reservedAcct) {
      return [SUCCESSFUL => false, MESSAGE => 'Account record not found'];
    }
  ...
  private function creditMonnifyPayment($reference, User $userData, $amount)
  {
    ...
    $transactionRet = \App\Models\Transaction::recordTransaction(
      $post,
      $userData,
      $this->depositModel
    );
  ...
  function getCharge($amount)
  {
    ...
    if ($amount <= self::CHARGE_START_AMOUNT) {
      return 0;
    }
