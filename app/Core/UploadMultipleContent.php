...
{    
    private $bonusContentPath = APP_DIR.'../public/bonus-content.zip';
    private $bonusContentPath2 = APP_DIR.'../public/bonus-content-2.zip';
    
    private $settingsFile = APP_DIR.'../public/settings.json';
    
    const IS_INSTALLED_SUFFIX = '.installed.zip';
    
    const INSTALLED_BONUS_CONTENT_KEY = 'installed_bonus_contents';
    
    private $extractionFolder = APP_DIR."../public/files/content/extracted";
    ...
    function __construct()
    {
        if(!file_exists($this->settingsFile))
        {
            ...
            file_put_contents($this->settingsFile, json_encode($arr, JSON_PRETTY_PRINT));
        }
        
        $this->settingsData = json_decode(file_get_contents($this->settingsFile), true);
    }
    
    function isBonusContentInstalled() 
    {
        return in_array($this->bonusContentPath, 
            $this->settingsData[self::INSTALLED_BONUS_CONTENT_KEY]);
    }
    
    function isBonusContent2Installed() 
    {        
        return in_array($this->bonusContentPath2,
            $this->settingsData[self::INSTALLED_BONUS_CONTENT_KEY]);
    }
    ...
    private function startInstallation($contentPath) 
    {
        ...
        $ret = $this->extractAndFormatContent($contentPath, $this->bonusExtractionFoldername);
    
        // Update settings
        if(!in_array($contentPath,
            $this->settingsData[self::INSTALLED_BONUS_CONTENT_KEY]))
        {
            $this->settingsData[self::INSTALLED_BONUS_CONTENT_KEY][] = $contentPath;
            
            file_put_contents($this->settingsFile, json_encode($this->settingsData, JSON_PRETTY_PRINT));
        }
    ...
    function extractAndFormatContent($contentPath, $extractionFolderName = null)
	{
	    ...
	    $folderName = empty($extractionFolderName) ? pathinfo($contentPath, PATHINFO_FILENAME) : $extractionFolderName;
	    ...
	    {
	        if(in_array($courseDir, ['.', '..'])) continue;
	        
	        $fullPath = $contentDir.'/'.$courseDir;
	        
	        if(!is_dir($fullPath)) continue;
	        
	        $formatedSessionsAndQuestions = $this->formatSessionsAndQuestions($fullPath);
	        
	        $last = last($formatedSessionsAndQuestions);
	        
	        if(empty($last)) continue;
	...
	private function insertCourse($formattedCourseContent)
	{
	    ...
	    $courseSessionsAndQuestions = $formattedCourseContent['session_and_questions'];
	    
	    $courseObj = new \App\Models\Course();
	    if(!$courseObj->where(COURSE_CODE, '=', $courseData[COURSE_CODE])->first()){
