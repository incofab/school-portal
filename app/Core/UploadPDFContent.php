...
{    
    ...
    const BASE_PATH = APP_DIR.'../'.self::FILE_PATH;
    const EXT = 'pdf';
    ...
    function uploadContent($files, $propertyName, $examName, $courseCode, $session) 
    {
        ...
        $fileAddressPath = self::FILE_PATH."$examName/$courseCode/{$ret['filename']}";
        
        return [SUCCESSFUL => true, MESSAGE => 'File uploaded', FILE_PATH => $fileAddressPath];
    }
    
    public function downloadContent($filePath)
    {
        $fileToDownload = realpath(APP_DIR.'../'.$filePath);
    ...
    private function uploadFile($files, $propertyName, $baseFolder,
        $filename = null, $maxSizeKilobytes = 500, 
        $ext = null)
    {
//         echo "propertyName = $propertyName <br />";
//         dDie($files);
        if(!isset($files[$propertyName])) return [SUCCESSFUL => false, MESSAGE => 'Invalid File'];
...
//         $validFileTypes = ['image/png', 'image/jpg', 'image/jpeg'];
        ...
        $filename = ($filename ? $filename : (pathinfo($name, PATHINFO_FILENAME).'-'.uniqid())).".$ext";
//         if(!in_array($fileType, $validFileTypes)) return [SUCCESSFUL => false, MESSAGE => 'Invalid file type'];
        ...
        {
            return [SUCCESSFUL => false, MESSAGE => "File greater than {$maxSizeKilobytes}kb"];
        }
        
        // Check if the file contains errors
        if($files[$propertyName]["error"] > 0) 
        {
            return [SUCCESSFUL => false, MESSAGE => "Return Code: " . $files[$propertyName]["error"]];
        }
        ...
        try 
        {
            ...
            if(!$success) return [SUCCESSFUL => false, MESSAGE => "Upload failed: Unknown error"];
        } 
        catch (\Exception $e) 
        {
            dlog('PDF uploadFile Error: '.$e->getMessage());
            
            return [SUCCESSFUL => false, MESSAGE => "Upload failed: ".$e->getMessage()];
        }
