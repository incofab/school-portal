...
class WhatsappWebhookHandler
{
  ...
  public function handle(array $payload): void
  {
    foreach ($this->messages($payload) as $message) {
      ...
      if (!$text) {
        $this->sender->sendTextMessage($from, $this->menuMessage());
        continue;
      }
  ...
  private function handleTextMessage(string $from, string $text): void
  {
    ...
    if ($this->state->get($from)) {
      $this->handleStudentSelection($from, $text);
      return;
    }

    if ($intent === WhatsappIntentResolver::CHECK_RESULT) {
      $this->startCheckResult($from);
      return;
    }
  ...
  private function handleStudentSelection(string $from, string $text): void
  {
    ...
    $this->state->clear($from);
    $this->sendResultResponse($from, $student);
  }

  private function startCheckResult(string $from): void
  {
    ...
    if ($students->count() === 1) {
      $this->sendResultResponse($from, $students->first());
      return;
    }
  ...
  private function sendResultResponse(string $from, Student $student): void
  {
    $response = $this->resultService->buildResultResponse($student);
